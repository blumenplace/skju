...
use crate::constants::{MAX_SAMPLE_COUNT, SAMPLE_BYTES, SAMPLE_RATE_HZ};
...
pub async fn handle_mpu_interrupts(
    ...
    if fifo_layout.sample_size != SAMPLE_BYTES {
    ...
    loop {
        ...
        let current_sample_count = current_sample_bytes as usize / SAMPLE_BYTES;
        let mut readings = [0x00; MAX_SAMPLE_COUNT * SAMPLE_BYTES];
        let current_timestamp = Instant::now().as_millis();

        if (current_sample_bytes as usize) < MAX_SAMPLE_COUNT * SAMPLE_BYTES {
            continue;
        }
        ...
        let batch_timestamp = current_timestamp - batch_span_millis;
...
pub async fn init_mpu(spim: Spim<'static>, mpu_cs: Output<'static>) -> MPU6500<SpiDeviceBus, TimerHandler> {
